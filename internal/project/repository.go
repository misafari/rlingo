...
func (r *Repository) FetchAll(ctx context.Context, tenantID uuid.UUID) ([]*projectDomain.Project, error) {
	tx, err := r.pool.Begin(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) FetchOneByID(ctx context.Context, projectID, tenantID uuid.UUID) (*projectDomain.Project, error) {
	tx, err := r.pool.Begin(ctx)
	if err != nil {
		return nil, err
	}
